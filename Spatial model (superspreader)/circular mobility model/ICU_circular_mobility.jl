...
function visualize(counter) #tested
    fig = plot()
    p1 = plot()
    p2 = plot()
    ...
    l_x = Float64[]
    l_y = Float64[]
    icu_x = Float64[] #FIXME: location of hospital discharge???
    icu_y = Float64[] #FIXME: central location for ICU
    ss_x = Float64[]
    ss_y = Float64[]
    ...
    for p in enumerate(pop)
        ...
        #plot!(p1, circleShape(a,b,r), seriestype = [:shape], lw = 0.5,c = :blue, linecolor = :black, fillalpha = 0.2, aspect_ratio = 1, label="")
    end

    plot!(p1, s_x, s_y, seriestype = :scatter, aspect_ratio = 0.8,
        #legend = false, xlims = (0.0, loc_l), ylims = (0.0, loc_l), label = "S")
        #legend = false, label = "S")
        legend = :outertopright, xlims = (0.0, hospital_x), ylims = (0.0, loc_l), label = "S", c=:blue,
        xticks=0:50:1.5*loc_l, yticks=0:50:loc_l)
    plot!(p1, e_x, e_y, seriestype = :scatter, label = "E", c = :pink)
    ...
    title!("λ="*string(λ)*", γ="*string(γ)*", tf="*string(tf)
    *", μ="*string(μ)*", σ="*string(σ))

    plot!(p2, S_vals, xlims = (0.0, tf*time_steps), ylims = (0.0, N)
        ,label = "S", legend=:outertopright, c=:blue)
    plot!(p2, I_vals, label = "I", c=:red)
    plot!(p2, R_vals, label = "R", c=:green)
    plot!(p2, E_vals, label = "E", c=:pink)
    plot!(p2, L_vals, label = "L", c=:orange)
    plot!(p2, ICU_vals, label = "ICU", c=:purple)
    xlabel!("time")
    ylabel!("Num ppl")
    ...
    display(fig)
    #=
    #code for saving to png files and convert to gif in terminal
    #included because of existing plotting error that still needs to be fixed
    if counter <10
        idx = "00"*string(counter)
    elseif counter <100
        idx = "0"*string(counter)
    else
        idx = string(counter)
    end
    #savefig("./ICU_fig/"*idx*".png")
    =#
...
function infect() #modified based on ss_strong_infectiousness.jl
    ...
    for p in enumerate(E) #E-->L
        event = generate_random_event(ρ*ζ) #
        if event == 1
            curr = p[2]
            delete!(E, curr)
            push!(L, curr)
    ...
    for p in enumerate(E) #E-->I
        event = generate_random_event(ζ*(1-ρ))
        if event == 1
            curr = p[2]
            delete!(E, curr)
            push!(I, curr)
...
function move() #move 1 entire circle
...
#gif(anim, "./figs/icu_1.gif", fps = 12)
